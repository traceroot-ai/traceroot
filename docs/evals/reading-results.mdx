...
`EvalRunResult` is what the process that ran the eval holds. Once the run has reported, the [CLI](/evals/cli) reads it back from anywhere — including a run somebody else started.

```bash
traceroot evals runs list --evaluation-id ev_…   # find the run id
traceroot evals runs get run_…                   # counts, per-scorer means, cost and duration
```
...
<CardGroup cols={2}>
  <Card title="From the Terminal" icon="terminal" href="/evals/cli">
    Read datasets, evaluations, and past runs with the CLI.
  </Card>
