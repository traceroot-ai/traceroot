...
| `sql [query]` | Run one read-only query over the project's spans and traces. See [Read-only SQL](/cli/sql). |
| `sql schema` | List every table and column available to read-only SQL. |
