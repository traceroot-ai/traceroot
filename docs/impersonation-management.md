...
- Entry: Support console is in the bottom-left account menu, visible only to admin/support outside impersonation. It is not a main sidebar navigation item. Account actions use left-aligned icons and labels.
- Users: 25 per page, partial email / exact user ID filter. Separate Name, Email and User ID columns, followed by Impersonate. No workspace column, expansion or copy button. Staff and banned accounts cannot be impersonated; the disabled button says why. Rows stay on screen while a new search loads.
...
- A compact amber strip at the top of the sidebar shows the truncated customer email and Stop. The collapsed sidebar retains an actionable exit icon; the account menu also provides Exit impersonation. No mode, workspace or reason is displayed. The indicator does not intercept browser requests: denied operations return a server-side 403 with an error message and `x-impersonation-denied`, and individual callers handle their errors. There is no universal denial notification. Exit restores the employee session and original filter/page. There is no impersonation-specific duration cap: ordinary login-session expiry/refresh applies (currently seven days of inactivity). Current employee/target eligibility is checked on every request. If the original employee session has expired, exit requires signing in again.
- A session that ends server-side (staff revoked, customer banned) unwinds itself: the next `get-session` runs the stop flow and restores the employee session when possible. `/admin` provides a recovery page, and the sidebar indicator shows Support session ended with a Back action. A missing employee session falls back to sign-in.
- Staff access: admin-only in both UI and API. Shows all existing accounts whose email ends in `@traceroot.ai` (case-insensitive), including accounts without access; no filter or pagination. Separate Name, Email and User ID columns and a compact role select (Admin / Support / No Access), with confirmation before a change. There is no Account column. Grants require a verified, non-banned account; no self-role changes. Email matching does not automatically grant access. The revoke API also permits revoking a previously privileged account whose email has changed, although that account is no longer listed. Revocation blocks subsequent reads; admin → support blocks subsequent writes. Already-dispatched requests are not canceled.
...
Application cookie API handlers use `withImpersonationPolicy`; a source-coverage test fails when an unwrapped route is added. Auth, internal validation and support endpoints have separate guards. The wrapper resolves the session once and supplies it and the impersonation context through request-local storage; `requireAuth` and billing handlers reuse that result, including a null session. Other callers resolve their own session. Nothing is cached across requests, and SSE live revalidation explicitly performs fresh lookups. Explicit credential/integration blocks cover both staff tiers, including CLI JWT exchange, device approvals, auth/account changes, GitHub token/OAuth, Slack installation, and API keys. JWT auto-issuance in `get-session` is disabled. Dashboard reads do not lazily create resources while impersonating.
...
![Users](images/impersonation/users.png)
![Staff access](images/impersonation/staff-access.png)
![Start session](images/impersonation/start-session.png)
![Sidebar indicator](images/impersonation/banner.png)

Regenerate from the current local UI with `UPDATE_SUPPORT_SCREENSHOTS=1 pnpm test:e2e -- e2e/impersonation.spec.ts` in `frontend/ui`.
