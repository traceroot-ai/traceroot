...
- A project with at least one incoming trace. See [Tracing](/tracing/get-started) if you don't have that yet.
- At least one supported detector model available to the project. TraceRoot Cloud provides system models automatically; self-hosted deployments need an admin to configure a supported system model environment variable such as `ANTHROPIC_API_KEY` or `OPENAI_API_KEY`, or a workspace owner can add a supported BYOK provider in **Settings → Model Providers**.
...
The create sheet offers a handful of templates covering common failure modes — things like tool errors, hallucinations, logic mistakes, incomplete tasks, and safety issues — plus a blank slate if none of them fit. Pick whichever maps to a problem you've seen in production. The template fills in a prompt you can edit.

The detector model picker only shows models that are actually available for the project. If the model catalog is still loading, unavailable, empty, or configured with only unsupported BYOK models, TraceRoot blocks creation and shows setup guidance instead of saving a detector that cannot run. Once a supported model is selected, save and the detector starts running on the next incoming trace.
...
3. **Evaluate.** The selected detector model reads the trace and returns `identified` plus a short summary. Omitted legacy/internal detector creates use a configured detector-safe default model when available.
