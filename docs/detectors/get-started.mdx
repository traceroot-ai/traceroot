...
5. **Notify.** At the end of the configured window, TraceRoot sends one digest to each configured email and Slack channel for findings from RCA-enabled detectors. The digest does not wait for RCA or include it.
