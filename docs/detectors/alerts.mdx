...
An alert fires once the two-step pipeline completes: the LLM-as-a-judge flags a trace, the TraceRoot AI agent finishes its root cause analysis, and the combined result lands in your configured channels. Email is supported by default; Slack is available on all plans, including Free.
...
Slack delivery is available on **all plans**, including Free. The integration is workspace-level — one Slack workspace per TraceRoot workspace, with a single destination channel.
