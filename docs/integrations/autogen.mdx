...
This integration instruments [AG2](https://github.com/ag2ai/ag2), the
community-maintained continuation that provides the `autogen` package:

```bash
pip install traceroot "ag2[gemini]"
```

Microsoft's newer `autogen-agentchat` package uses a different API and is not
currently supported by this integration.

To capture the complete picture—including both the agent orchestrations and the underlying token costs—we highly recommend initializing both the AG2 integration and your specific LLM provider (e.g., Google GenAI, OpenAI, Anthropic).
