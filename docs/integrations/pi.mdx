...
`captureContent` can also be a function that redacts, truncates, replaces, or omits each prompt and response before it is recorded. See [Data Capture and Privacy](/tracing/data-capture) for a working redaction example.
