...
OpenAI, Anthropic, Azure OpenAI, Google (Gemini), Amazon Bedrock, xAI, DeepSeek, OpenRouter, Kimi, GLM, TypeSafe AI.

TypeSafe AI is for detectors only; it never appears in the AI chat or RCA model pickers. After selecting it, click **Add Model** to add `jev-1.13.0`, then pick that model in a detector. Jev returns a category with its probabilities and a confidence, not written text, so a finding reads like `Jev: tool_error (P=0.91, label confidence 0.88)`.
