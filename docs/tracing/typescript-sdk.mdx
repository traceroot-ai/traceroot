...
**Important:** The SDK's only automatic flush is a `beforeExit` hook. Node.js does not emit `beforeExit` on `process.exit()`, unhandled exceptions, or signals like SIGINT/SIGTERM. Serverless runtimes that freeze the process (e.g. AWS Lambda, Cloud Functions) skip it too. In any of these cases, spans will be lost if you exit without flushing explicitly.
...
async function main() {
  try {
    ...
    // Ensure all batched spans are exported before exiting, even on failure
...
Alternatively, for tests or short scripts, you can disable batching:

```typescript
TraceRoot.initialize({ disableBatch: true });
```

**Note:** Disabling batching does not make the network request synchronous. You still must await `flush()` or `shutdown()` before terminating the process with `process.exit()`.

*(Note: The Python SDK registers an `atexit` handler that flushes spans automatically in most environments. However, `atexit` does not run when a serverless runtime freezes the process — Python handlers there still need `traceroot.flush()` before returning.)*
