...
class ContextCacheSettings:
    ...
    def from_env(cls) -> "ContextCacheSettings":
        ...
        return cls(
            enabled=os.getenv("TRACEROOT_CACHE_ENABLED",
                              "true").lower() == "true",
            strategy=CacheStrategy(os.getenv("TRACEROOT_CACHE_STRATEGY",
                                             "append_only")),
            max_cache_size=int(os.getenv("TRACEROOT_CACHE_MAX_SIZE",
                                         "100")),
            max_context_size=int(
                os.getenv("TRACEROOT_CACHE_MAX_CONTEXT_SIZE",
                          "1000000")
            ),
            ttl_seconds=int(os.getenv("TRACEROOT_CACHE_TTL_SECONDS",
                                      "3600")),
            enable_append_only=os.getenv("TRACEROOT_CACHE_APPEND_ONLY",
                                         "true").lower() == "true",
            hash_algorithm=os.getenv("TRACEROOT_CACHE_HASH_ALGORITHM",
                                     "sha256"),
        )
