...
try:
    from rest.utils.ee.auth import (
        generate_traceroot_token,
        generate_user_credentials,
        get_user_credentials,
        hash_user_sub,
    )
except ImportError:
    from rest.utils.auth import (
        generate_traceroot_token,
        generate_user_credentials,
        get_user_credentials,
        hash_user_sub,
...
class IntegrateRouter:
    ...
    def _setup_routes(self):
        ...
        self.router.post("")(
            self.limiter.limit(self.rate_limit_config.post_integrate_limit
                               )(self.post_integrate)
        )
        self.router.get("")(
            self.limiter.limit(self.rate_limit_config.get_integrate_limit
                               )(self.get_integrate)
        )
        self.router.delete("")(
            self.limiter.limit(self.rate_limit_config.delete_integrate_limit
    ...
    async def post_integrate(
        ...
        try:
            ...
            if (
                not self.local_mode and req_data.resource_type == ResourceType.TRACEROOT
            ):
                token = generate_traceroot_token()
                # Get user_sub for AWS role session
                user_credentials = generate_user_credentials(user_sub, user_email)
            ...
            elif (self.local_mode and req_data.resource_type == ResourceType.TRACEROOT):
                # return a fake token for local mode, don't insert into DB
                token = "fake_traceroot_token_for_local_mode"
                return IntegrateResponse(success=True, token=token).model_dump()
            else:
        ...
        try:
            ...
            if req_data.resource_type != ResourceType.TRACEROOT:
                ...
                success = await self.db_client.delete_integration_token(
                    user_email=user_email,
                    token_type=req_data.resource_type
                )
            else:
                hashed_user_sub = hash_user_sub(user_sub)
                # Delete the token from database
