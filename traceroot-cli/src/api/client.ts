...
export class ApiClient {
  ...
  private async request<T>(method: string, path: string, body?: unknown): Promise<T> {
    ...
    // Guard against empty bodies not caught by content-length (e.g. chunked
    // transfer with empty payload, proxies that strip headers).  response.json()
    // throws SyntaxError on empty input.
