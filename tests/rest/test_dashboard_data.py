...
def test_internal_mirror_answers_like_the_public_route(monkeypatch):
    ...
    with patch(RUN, return_value=_rows(1)):
        ...
        internal = TestClient(app).get(
            "/api/v1/internal/projects/proj-A/dashboards/dash-1/data",
            headers={"X-Internal-Secret": "test-secret", "x-user-id": "u1"},
            params=params,
        )
    assert public.status_code == internal.status_code == 200
    public_body, internal_body = public.json(), internal.json()
    # The one deliberate difference: the secret-authed mirror keeps the
    # creator that an API-key caller is not shown (see the tests below).
    assert public_body["dashboard"].pop("creator") is None
    assert internal_body["dashboard"].pop("creator") == "Ada Lovelace"
    assert public_body == internal_body


@respx.mock
def test_api_key_caller_is_not_told_who_created_the_dashboard():
    _mock_key_auth()
    _mock_detail(_detail([_widget(1)]))
    with patch(RUN, return_value=_rows(1)):
        response = TestClient(app).get(PATH, headers=KEY_HEADER)
    assert response.status_code == 200
    assert response.json()["dashboard"]["creator"] is None


@respx.mock
def test_user_caller_sees_the_creator():
    _mock_user_auth()
    _mock_detail(_detail([_widget(1)]))
    with patch(RUN, return_value=_rows(1)):
        response = TestClient(app).get(PATH, headers=USER_HEADER, params={"project_id": "proj-A"})
    assert response.status_code == 200
    assert response.json()["dashboard"]["creator"] == "Ada Lovelace"
