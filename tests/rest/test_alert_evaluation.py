...
class _FakeClickHouse:
    ...
    def query(self, sql, parameters=None, settings=None):
        ...
        assert self._values, f"unexpected extra query: {sql}"
        return _scalar_result(self._values.pop(0))
...
def _scalar_result(value):
    rows = [] if value is _NO_ROWS else [(value,)]
    return MagicMock(column_names=["value"], result_rows=rows)
...
def test_count_measure_issues_exactly_one_query(fake_ch):
    """The measure already is the row count; a second sentinel query would double its cost."""
    ch = fake_ch(137)
...
def test_non_count_measure_issues_the_sentinel_alongside_the_measure(fake_ch):
    ch = fake_ch(250.0, 9)
    result = evaluate([make_alert()])[0]

    assert len(ch.calls) == 2
    assert "avg(duration_ms)" in ch.sqls[0]
    assert "count(*)" in ch.sqls[1]
    assert result.value == 250.0
    assert result.row_count == 9


def test_sentinel_measures_the_same_window_and_filters_as_the_measure(fake_ch):
    ...
    measure_params, sentinel_params = ch.calls[0][1], ch.calls[1][1]
    assert measure_params == sentinel_params
    assert sentinel_params["start_time"] == START
    assert sentinel_params["end_time"] == END
    assert sentinel_params["f0"] == "LLM"
