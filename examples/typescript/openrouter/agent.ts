/**
 * OpenRouter ReAct Agent — TraceRoot Observability
 *
 * Uses the OpenAI SDK pointed at OpenRouter's API endpoint. TraceRoot's OpenAI
 * integration captures all calls automatically — no extra setup needed.
 *
 * Env vars required: OPENROUTER_API_KEY, TRACEROOT_API_KEY
 *
 * Run:
 *   pnpm demo
 */
...
Available tools: weather, stock prices, calculator, current time.`;
...
async function main() {
  try {
    await usingAttributes(
      {
        sessionId: 'openrouter-demo-session',
        userId: 'demo-user',
        tags: ['demo', 'openrouter', 'react-agent'],
        metadata: { example: 'openrouter-react-agent', sdkFeature: 'usingAttributes' },
      },
      () =>
        observe({ name: 'demo_session' }, async () => {
          console.log('='.repeat(60));
          console.log('OpenRouter ReAct Agent — Demo (TraceRoot)');
          console.log('='.repeat(60));
