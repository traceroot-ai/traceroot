...
-- reads can use it — and prune on it.
--
...
--
-- Adding `source` to the projection makes those reads eligible again. Placing it
-- SECOND in the ORDER BY (after project_id, before span_start_time) does more
-- than restore eligibility: it physically segregates detector rows from customer
-- rows within each project, so `source = 'user'` prunes the detector rows out at
-- the index level instead of reading and discarding them. project_id stays first
-- to match every other sort key in the schema. Validated on a scratch copy: a
-- source-filtered time-window query uses the rebuilt projection and prunes to a
-- single granule.
--
...
(
    SELECT
        ...
        source
    ORDER BY (project_id, source, span_start_time, trace_id, span_id)
);
