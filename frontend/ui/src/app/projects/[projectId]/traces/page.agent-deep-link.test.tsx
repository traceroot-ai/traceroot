// @vitest-environment jsdom
import { afterEach, describe, expect, it, vi } from "vitest";
import { cleanup, fireEvent, render, screen } from "@testing-library/react";
import { QueryClient, QueryClientProvider } from "@tanstack/react-query";
import React from "react";

// The URL the viewer's "open in new tab" button builds while showing an RCA
// agent trace (the analysis hop from the Alert chip, or the sidebar sheet).
const AGENT_TRACE_ID = "f1f1";
let search = `traceId=${AGENT_TRACE_ID}&source=agent&fullscreen=1`;

vi.mock("@/features/traces/hooks", () => ({
  useTraces: () => ({
    data: {
      data: [
        { trace_id: "t1", name: "n", trace_start_time: 0, error_count: 0, span_count: 1 },
        { trace_id: "t2", name: "n", trace_start_time: 0, error_count: 0, span_count: 1 },
      ],
      meta: { page: 1, limit: 50, total: 2 },
    },
    isLoading: false,
    error: null,
  }),
  useSpanIO: () => ({ data: undefined, isLoading: false, error: null }),
  useTracesExist: () => ({ data: { exists: true }, isPending: false, error: null }),
  usePrefetchTraces: () => vi.fn(),
}));
vi.mock("@/lib/hooks/use-list-page-state", () => ({
  useListPageState: () => ({
    state: { dateFilter: "all", customStartDate: null, customEndDate: null, keyword: "" },
    goToPage: vi.fn(),
    updateLimit: vi.fn(),
    updateDateFilter: vi.fn(),
    updateCustomRange: vi.fn(),
    updateKeyword: vi.fn(),
    queryOptions: { page: 1, limit: 50 },
  }),
}));
vi.mock("@/lib/hooks/use-local-storage", () => ({
  useLocalStorage: () => [false, vi.fn()],
}));
vi.mock("@/lib/auth-client", () => ({
  useSession: () => ({ data: { user: { id: "u1", email: "e" } }, isPending: false }),
}));
vi.mock("next/navigation", () => ({
  useParams: () => ({ projectId: "p1" }),
  useSearchParams: () => new URLSearchParams(search),
  useRouter: () => ({ replace: vi.fn(), push: vi.fn() }),
}));
vi.mock("@/components/layout/app-layout", () => ({
  useLayout: () => ({ setHideAiButton: vi.fn() }),
}));
vi.mock("@/features/projects/components", () => ({ ProjectBreadcrumb: () => null }));
vi.mock("@/lib/hooks/use-retention", () => ({
  useRetention: () => ({
    retentionDays: 15,
    showPricing: false,
    onUpgradeClick: vi.fn(),
    closePricing: vi.fn(),
    workspaceId: "ws-1",
    billingPlan: "free",
  }),
}));
vi.mock("@/ee/features/billing/PricingDialog", () => ({ PricingDialog: () => null }));
// The panel mock surfaces the props under test and lets the page's own
// navigation move the selection off the deep-linked id.
vi.mock("@/features/traces/components", () => ({
  TraceViewerPanel: ({
    traceId,
    source,
    onNavigate,
    onClose,
  }: {
    traceId: string;
    source?: string;
    onNavigate: (d: "up" | "down") => void;
    onClose: () => void;
  }) => (
    <div data-testid="trace-panel" data-trace-id={traceId} data-source={String(source)}>
      <button type="button" onClick={() => onNavigate("down")}>
        panel-down
      </button>
      <button type="button" onClick={onClose}>
        panel-close
      </button>
    </div>
  ),
  GettingStarted: () => null,
}));
vi.mock("@/components/search-filter-bar", () => ({
  SearchFilterBar: ({ children }: { children: React.ReactNode }) => <div>{children}</div>,
}));
vi.mock("@/features/traces/utils", () => ({ formatContentPreview: () => "" }));
vi.mock("next/link", () => ({
  default: ({ children, ...props }: { children: React.ReactNode; href: string }) => (
    <a {...props}>{children}</a>
  ),
}));

import TracesPage from "./page";

function Wrapper({ children }: { children: React.ReactNode }) {
  const client = new QueryClient({ defaultOptions: { queries: { retry: false } } });
  return <QueryClientProvider client={client}>{children}</QueryClientProvider>;
}

afterEach(() => {
  cleanup();
  search = `traceId=${AGENT_TRACE_ID}&source=agent&fullscreen=1`;
});

describe("TracesPage ?traceId=&source=agent deep link (popout from an agent view)", () => {
  it("opens the deep-linked id under the agent scope", () => {
    render(<TracesPage />, { wrapper: Wrapper });

    const panel = screen.getByTestId("trace-panel");
    expect(panel.getAttribute("data-trace-id")).toBe(AGENT_TRACE_ID);
    // Without this the popped-out tab asked for the agent trace under the
    // customer scope and 404'd with "Error loading trace".
    expect(panel.getAttribute("data-source")).toBe("agent");
  });

  it("drops back to the customer scope as soon as the selection leaves the deep-linked id", () => {
    render(<TracesPage />, { wrapper: Wrapper });

    // The agent id matches no list row, so "down" selects the first row.
    fireEvent.click(screen.getByRole("button", { name: "panel-down" }));

    const panel = screen.getByTestId("trace-panel");
    expect(panel.getAttribute("data-trace-id")).toBe("t1");
    expect(panel.getAttribute("data-source")).toBe("user");
  });

  it("ignores source for a deep link that names any other scope", () => {
    search = "traceId=t1&source=detector";
    render(<TracesPage />, { wrapper: Wrapper });

    const panel = screen.getByTestId("trace-panel");
    expect(panel.getAttribute("data-trace-id")).toBe("t1");
    expect(panel.getAttribute("data-source")).toBe("user");
  });
});
