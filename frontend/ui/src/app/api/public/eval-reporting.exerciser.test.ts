...
describe("SDK reporting: run provenance is gone (SDK-agnostic identity)", () => {
  it("rejects a run that still sends the removed `provenance` block", async () => {
    // Evaluation identity no longer records which SDK produced a run, so `provenance` is
    // not a modelled field. The strict register contract rejects it outright rather than
    // silently persisting SDK identity (no released SDK sends it — it was unshipped).
    const reg = await registerRun(
      req({
        evaluation_name: "Legacy-provenance eval",
        dataset_id: "ds1",
        candidate_version: "git:abc123",
        provenance: { sdk_language: "python", git_commit: "4a91c02" },
      }),
    );
