...
function TimeInput({ label, date, onChange }: TimeInputProps) {
  ...
  const [month, setMonth] = useState(() =>
    date ? String(date.getUTCMonth() + 1).padStart(2, "0") : "",
  );
  ...
  const [minute, setMinute] = useState(() =>
    date ? String(date.getUTCMinutes()).padStart(2, "0") : "",
  );
  const [period, setPeriod] = useState<"AM" | "PM">(() => init12.period);

  const updateDate = useCallback(
    (y: string, m: string, d: string, h: string, min: string, p: "AM" | "PM") => {
  ...
  const inputBase =
    "h-7 text-center text-[13px] border border-border rounded bg-background focus:outline-none focus:ring-1 focus:ring-ring";
    ...
    <div className="mb-3">
      ...
      <div className="flex items-center gap-1">
        <input
          ...
          value={year}
          onChange={(e) => {
            handleNum(setYear, e.target.value, 4);
            updateDate(e.target.value, month, day, hour, minute, period);
          }}
          onBlur={correctYear}
          placeholder="yyyy"
        ...
        <input
          ...
          value={month}
          onChange={(e) => {
            handleNum(setMonth, e.target.value, 2);
            updateDate(year, e.target.value, day, hour, minute, period);
          }}
          onBlur={correctMonth}
          placeholder="mm"
        ...
        <input
          ...
          value={day}
          onChange={(e) => {
            handleNum(setDay, e.target.value, 2);
            updateDate(year, month, e.target.value, hour, minute, period);
          }}
          onBlur={correctDay}
          placeholder="dd"
          className={cn(inputBase, "w-[32px]", !day && "text-muted-foreground")}
        />
        <span className="mx-1 text-[13px] text-muted-foreground">,</span>
        <input
          type="text"
          value={hour}
          onChange={(e) => {
            handleNum(setHour, e.target.value, 2);
            updateDate(year, month, day, e.target.value, minute, period);
          }}
          onBlur={correctHour}
          placeholder="12"
        ...
        <input
          ...
          value={minute}
          onChange={(e) => {
            handleNum(setMinute, e.target.value, 2);
            updateDate(year, month, day, hour, e.target.value, period);
          }}
        ...
        <button
          ...
          className="ml-1 flex h-7 items-center gap-0.5 rounded border border-border bg-background px-2 text-[13px] hover:bg-muted"
        >
          {period}
          <ChevronDown className="h-3 w-3 text-muted-foreground" />
        </button>
        <span className="ml-1.5 text-[13px] text-muted-foreground">UTC</span>
      </div>
    </div>
  );
}
