-- Cost-derivation convergence: a `cost_derived_at` marker distinct from `cost IS NULL`.
--
...
--    the whole table every ~10 min. Partial/WHERE indexes can't be expressed in
