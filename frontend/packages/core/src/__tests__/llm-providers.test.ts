import { BYOK_DOC_END, BYOK_DOC_START, renderByokModelsSection } from "../byok-docs";
...
describe("the generated BYOK model region is current", () => {
  // The region is written by scripts/generate-byok-models-doc.ts. This fails if the
  // catalog changed without regenerating, so the docs cannot drift from the code.
  const BYOK_DOC = new URL("../../../../../docs/ai-agent/byok.mdx", import.meta.url);

  it("matches what the generator would write", () => {
    const doc = readFileSync(BYOK_DOC, "utf8");
    const start = doc.indexOf(BYOK_DOC_START);
    const end = doc.indexOf(BYOK_DOC_END);

    expect(start, `${BYOK_DOC_START} missing from docs/ai-agent/byok.mdx`).toBeGreaterThan(-1);
    expect(end, `${BYOK_DOC_END} missing from docs/ai-agent/byok.mdx`).toBeGreaterThan(-1);

    expect(
      doc.slice(start, end + BYOK_DOC_END.length),
      "docs/ai-agent/byok.mdx is stale — run pnpm --filter @traceroot/core generate:byok-docs",
    ).toBe(renderByokModelsSection());
  });
});
