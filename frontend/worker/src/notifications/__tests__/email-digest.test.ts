...
describe("sendDigestAlertEmail", () => {
  ...
  it("sends a digest email with subject, window-filtered deep-links, and subline", async () => {
    ...
    expect(mail.text).toContain("- Hallucination — 2 triggers");
    expect(mail.text).toContain("- Latency spike — 1 trigger");
    expect(mail.text).not.toContain("- Hallucination — 2 findings");
  });
  ...
  it("is byte-identical to today when summary is absent", async () => {
    ...
    // Compare against the no-summary capture so a template regression cannot
    // hide behind a blank-vs-absent equality if both outputs drift together.
    expect(without.html).toBe(NO_SUMMARY_BASELINE_HTML);
    expect(without.text).toBe(NO_SUMMARY_BASELINE_TEXT);
  });
});
