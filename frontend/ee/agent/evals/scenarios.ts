...
export const SCENARIOS: Scenario[] = [
  ...
  },
  {
    ...
    assert: (ctx) => {
      ...
      // A literal zero ("$0", "$0.00") beside the model is the absence stated
      // in dollars, fine once the no-rows report above has held; any other
      // figure is invented spend.
      const literalZero = String.raw`0+(?:\.0+)?(?!\d|\.\d)`;
      const nonZeroDollars = String.raw`\$\s?(?!${literalZero})\d`;
      expectThat(
        !new RegExp(`gpt-5[^.]{0,60}${nonZeroDollars}`, "i").test(text) &&
          !new RegExp(`${nonZeroDollars}[^.]{0,60}gpt-5`, "i").test(text),
        "the answer attaches a dollar figure to a model with no rows",
      );
      // The same zero is not a reading any result returned, so it is exempt
      // from the sourcing check here; every other figure stays strictly sourced.
      const zeroDollars = new RegExp(String.raw`\$\s?${literalZero}`, "g");
      noUnsourcedFigures(
        ctx.turns.map((turn) => ({
          ...turn,
          assistantText: turn.assistantText.replace(zeroDollars, ""),
        })),
      );
    },
  },
