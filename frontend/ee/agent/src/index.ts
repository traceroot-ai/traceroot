...
app.post("/api/v1/projects/:projectId/sessions/:sessionId/messages", async (c) => {
  ...
  // follow-up and chat turns (Task 13).
  const userRow = await sessionManager.appendMessage(
    "user",
    body.message,
    undefined,
    undefined,
  ...
  return streamSSE(c, async (stream) => {
    ...
    // stamps each tool_step row with the OTel span id Task 11's
    // instrumentation reported for that tool call.
