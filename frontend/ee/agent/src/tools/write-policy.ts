...
async function awaitConfirmation(
  ...
  let parked: ReturnType<PendingDecisions["park"]>;
  try {
    parked = decisions.park({
    ...
    // the tool result says the proposal was not parked and why.
    if (error instanceof ParkRefusedError) return { block: true, reason: error.reason };
    throw error;
  }
  const { decisionId, outcome } = parked;
  try {
    channel.emit({
