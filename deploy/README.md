...
### ACM certificate stuck "Still creating..."

DNS is not delegated. Run `terraform output nameservers` and set up NS records
at your DNS provider (Step 3). The cert validates via DNS — if Route53 can't
serve the validation records, it will hang forever.

### `ImagePullBackOff` on migration pods

ECR repos are empty. Build and push images (Step 4), then `terraform apply` again.

### `storageclasses is forbidden` or kubectl auth errors

Your kubeconfig is stale. Run:
```bash
aws eks update-kubeconfig --name traceroot --region us-east-1
```

### Helm release failed pre-install

Migration jobs timed out. Check why:
```bash
kubectl get pods -n traceroot-staging
kubectl logs <migrate-pod-name> -n traceroot-staging
```

Common causes: ImagePullBackOff (push images first), database not ready
(re-run `terraform apply`).

## Architecture

```
terraform.tfvars          # Your config (domain, API keys, replicas)
    |
    v
variables.tf              # Input schema (types, defaults, validation)
    |
    v
secrets.tf                # Auto-generates passwords, creates k8s Secrets
traceroot.tf              # Helm release — wires Terraform outputs into chart
dns.tf                    # Route53 zone, ACM cert, ALB DNS record
    |
    v
values.yaml               # Helm chart defaults (Terraform overrides these)
    |
    v
templates/*.yaml           # Kubernetes manifests (pods, services, ingress)
```

See `docs/plans/2026-03-09-terraform-helm-env-var-architecture.md` for the
full env var mapping between `.env.example` and the hosting setup.
