...
printf '%s' "$SHOW_OUT" | grep -qE "DEFINER = [A-Za-z0-9_]+ SQL SECURITY DEFINER" \
  || { echo "FAIL: no explicit definer recorded"; exit 1; }
echo "PASS: definer recorded -> $(printf '%s' "$SHOW_OUT" | grep -oE 'DEFINER = [A-Za-z0-9_]+ SQL SECURITY DEFINER')"
...
echo "== RO denied on the physical table (EXPECTED-DENY) =="
if ch_ro --query "SELECT count() FROM pubviews.spans" >/dev/null 2>&1; then
